...
const NavBar = (props) => {
    ...
    const isLoginOrRegister = location.pathname === '/login' || location.pathname === '/registeruser';
            ...
            <div className={style.mainContent}>
                <div className={style.linkContainer}>
                    ...
                    {!isLoginOrRegister && (
                        <>
                            ...
                            {admin && <Link to="/dashboard" className={style.linkDesk} onClick={toggleNav}>Dashboard</Link>}
                        </>
                    )}
                </div>
                ...
                {!isLoginOrRegister && (
                    <div className={style.searchDeskContent}>
                        <div className={style.group}>
                            <input required type="text" placeholder='Buscar producto' className={style.input} value={search} onChange={handleChange} />
                ...
                <div className={style.cartContainer}>
                    {userState === null ? (
                        ...
                        <button onClick={cerrarSesion} className={style.buttonSign}>Cerrar sesión</button>
                    )}
                </div>
                ...
                {!isLoginOrRegister && (
                    <div className={style.cartContainer}>
                        {userState !== null && (
                            <>
                                <img className={style.iconPerfil} src='https://cdn.icon-icons.com/icons2/3298/PNG/96/ui_user_profile_avatar_person_icon_208734.png' alt="perfil" />
                                <p>{userState.name}</p>
                            </>
                        )}
